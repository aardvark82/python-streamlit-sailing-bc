...
# ── Day/night shading helpers (dependency-free sun calc) ──────────────

def _sunrise_sunset_ut(year, month, day, lat, lon):
    """NOAA / Almanac sunrise equation. Returns (sunrise_ut, sunset_ut) as
    fractional hours UTC for the given date, or None where the sun doesn't
    rise/set (not a concern at Vancouver's latitude)."""
    import math
...
def _sun_event_local(d, ut, tz):
    ...
    datetime, snapping it back onto local date d (corrects the UTC date drift
    that otherwise puts Vancouver sunset on the wrong calendar day)."""
    if ut is None:
        return None
...
def _add_day_night_shading(fig, start, end, lat=49.28, lon=-123.12):
    """Shade the plot background: normal blue for daylight, darker blue for
    night, across the [start, end] window. Bands sit BELOW the data."""
    tz = pytz.timezone('America/Vancouver')
    start = start.astimezone(tz)
    end = end.astimezone(tz)

    # Daylight intervals per calendar day in the window (±1 day of slack)
    day_intervals = []
    ...
    # Night intervals = complement of daylight within the window
    night_intervals = []
    cursor = start
    ...
    for a, b in day_intervals:
        fig.add_shape(type='rect', xref='x', yref='paper',
                      ...
                      fillcolor=DAY, line_width=0, layer='below')
    for a, b in night_intervals:
        fig.add_shape(type='rect', xref='x', yref='paper',
                      x0=pd.Timestamp(a), x1=pd.Timestamp(b), y0=0, y1=1,
                      fillcolor=NIGHT, line_width=0, layer='below')
...
def plot_wave_history_chart(container, past_df, buoy_id):
    ...
    fig_wave = go.Figure()
    # Day/night background bands first so they render beneath the line.
    try:
        _add_day_night_shading(fig_wave, three_days_ago, now_van)
    except Exception as e:
        print(f"Day/night shading skipped: {e}")
